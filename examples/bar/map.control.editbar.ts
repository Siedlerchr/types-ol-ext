...
(edit.getInteraction('ModifySelect') as ModifyFeature).on('modifystart', function (e) {
  if (e.features.length === 1) tooltip.setFeature(e.features[0]);
});
(edit.getInteraction('ModifySelect') as ModifyFeature).on('modifyend', function (e) {
  tooltip.setFeature();
});
edit.getInteraction('DrawPoint').on('change:active', function (e) {
  tooltip.setInfo(e.oldValue ? '' : 'Click map to place a point...');
});
(edit.getInteraction('DrawLine') as Draw).on('change:active', function (e) {
  tooltip.setFeature();
  tooltip.setInfo(e.oldValue ? '' : 'Click map to start drawing line...');
});
(edit.getInteraction('DrawLine') as Draw).on('drawend', function (e) {
  tooltip.setFeature();
  // oldValue doesn't exist in DrawEvent
  //tooltip.setInfo(e.oldValue ? '' : 'Click map to start drawing line...');
});
(edit.getInteraction('DrawLine') as Draw).on('drawstart', function (e) {
  tooltip.setFeature(e.feature);
  tooltip.setInfo('Click to continue drawing line...');
});
(edit.getInteraction('DrawPolygon') as Draw).on('drawstart', function (e) {
  tooltip.setFeature(e.feature);
  tooltip.setInfo('Click to continue drawing shape...');
});
(edit.getInteraction('DrawPolygon') as Draw).on('change:active', function (e) {
  tooltip.setFeature();
  tooltip.setInfo(e.oldValue ? '' : 'Click map to start drawing shape...');
});
(edit.getInteraction('DrawPolygon') as Draw).on('drawend', function (e) {
  tooltip.setFeature();
  //tooltip.setInfo(e.oldValue ? '' : 'Click map to start drawing shape...');
});
(edit.getInteraction('DrawHole') as DrawHole).on('drawstart', function (e) {
  tooltip.setFeature(e.feature);
