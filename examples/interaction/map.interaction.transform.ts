...
import {
  Style, RegularShape, Fill, Stroke, Text,
} from 'ol/style'
...
import type { KeyEvent } from 'ol-ext/interaction/CurrentMap'
...
export function setAspectRatio(p: string) {
  if ($(`#${p}`).prop('checked')) interaction.set('keepAspectRatio', always)
    ...
    interaction.set('keepAspectRatio', (e: KeyEvent) => {
...
interaction.on('select', (e: SelectEvent) => {
...
interaction.on(['rotatestart', 'translatestart', 'scalestart'], (e: any) => {
  // Rotation
