...
const popup1 = new Popup({ anim: true, closeBox: true })
map.addOverlay(popup1)
const popup2 = new Popup({ anim: true, closeBox: true })
...
select1.getFeatures().on('add', e => {
  showPopup(e, popup1)
})
select2.getFeatures().on('add', e => {
  showPopup(e, popup2)
})
...
// Tools
const clip = new Clip({ radius: 150, layers: map2.getLayers().getArray() })
// var swipe = new ol.control.Swipe({ rightLayers: map2.getLayers().getArray(),  });
const swipe = new SwipeMap({ right: true })
