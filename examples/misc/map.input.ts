...
let switcher = new Switch({ input: $('.switch').get(0), html: 'off', after: 'on' });

const checkbox = new Checkbox({ input: $('.check').get(0), html: 'Checkbox:', after: 'on' });
$('.radio').each(function(i: number): void {
  window[`radio${i + 1}`] = new Radio({ input: this, html: i ? '' : 'Radio:', after: 'radio ' + (i + 1), checked: !i });
});
