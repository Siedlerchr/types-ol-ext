...
for (const font in FontSymbol.prototype.defs.fonts) {
    ...
    for (const i in glyph) {
        if (glyph[i].font == font) {
            ...
            item.addEventListener("click", () => { setFont(item) });
...
function getFeatureStyle (feature: FeatureLike): Style[] {
    const st= [];
    // Shadow style
    if (document.querySelector<HTMLInputElement>("#shadow")!.checked) st.push ( new Style({
        image: new Shadow({
            radius: 15,
            blur: 5,
            offsetX: 0,
...
//
function setOptions(glyph: string, form: string, color: string, scolor: string, fcolor: string, border: number,
                    radius: number, fsize: number, offset: boolean, gradient: boolean, shadow: boolean) {
    theGlyph = glyph;
    theText = '';
...
window.setOptions = (glyph: string, form: string, color: string, scolor: string, fcolor: string, border: number,
                     radius: number, fsize: number, offset: boolean, gradient: boolean, shadow: boolean): void => {
    setOptions(glyph, form, color, scolor, fcolor, border, radius, fsize, offset, gradient, shadow);
};
