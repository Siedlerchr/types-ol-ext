...
$(window).on('load', () => {
       ...
       FillPattern.addPattern("bug (fontawesome)", { char: '\uf188', size: 12, font: "10px FontAwesome" });
        FillPattern.addPattern("smiley (width angle)", { char: '\uf118', size: 20, angle: true, font: "15px FontAwesome" });
        ...
        for (const i in FillPattern.prototype.patterns) {
            ...
            $("<div>").attr('title', i)
                ...
                .click(function() { pat = $(this).attr("title") as string ; refresh(); })
                .appendTo(spattern);
        }
